...
describe('DeleteCommentUseCase', () => {
  it('it should orchestrate the delete comment action correctly', async () => {
    ...
    await expect(deleteCommentUseCase.execute(useCasePayload)).resolves.not.toThrowError(Error);

    expect(mockThreadRepository.verifyThreadIsExists).toBeCalledWith(useCasePayload.threadId);
    expect(mockCommentRepository.verifyCommentIsExists).toBeCalledWith(useCasePayload.id);
